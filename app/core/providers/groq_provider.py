...
class GroqProvider(BaseProvider):
    ...
    def chat(self, messages: list[ChatMessage], model: str | None = None, temperature: float = 0.0, max_tokens: int = 1024) -> CompletionResult:
        ...
        response = self._client.chat.completions.create(
            model=model,
            messages=cast(list[ChatCompletionMessageParam], to_chat_message_params(messages)),
        ...
        return CompletionResult(content=choice.message.content or "", model=response.model, usage=usage_dict)

    def list_models(self) -> list[str]:
        """
        Returns available model ID's for this Groq (limited to free tier for now).
        """
